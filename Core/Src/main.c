...
int main(void)
{
  ...
  /* Initialize the SD before USB enumeration. This prevents Windows from
     seeing an initial "no media" state and entering a long retry delay. */
  MX_SDIO_SD_Init();
  ...
  if (((usb_disk_status & STA_NOINIT) == 0) &&
  ...
  while ((hUsbDeviceFS.dev_state != USBD_STATE_CONFIGURED) &&
         ((HAL_GetTick() - usb_check_start) < 2500))
  {
      HAL_Delay(10);
  }
  ...
  if (hUsbDeviceFS.dev_state == USBD_STATE_CONFIGURED)
  {
      ...
      SSD1306_WriteString("LOGGING STOPPED");
      ...
      while (1)
      {
          uint32_t usb_blocks;
          uint32_t usb_errors;
          uint32_t usb_verify_errors;

          USB_Storage_GetWriteStats(&usb_blocks, &usb_errors,
                                    &usb_verify_errors);
          if ((usb_errors != 0U) || (usb_verify_errors != 0U))
          {
              SSD1306_SetCursor(0, 2);
              SSD1306_WriteString("USB WRITE ERROR ");
              SSD1306_SetCursor(0, 4);
              snprintf(msg, sizeof(msg), "ERR=%lu VERIFY=%lu ",
                       usb_errors, usb_verify_errors);
          ...
          else
          {
              ...
              SSD1306_WriteString("SD CARD ON PC   ");
              SSD1306_SetCursor(0, 4);
              SSD1306_WriteString((usb_blocks != 0U) ?
                                  "WRITES VERIFIED " :
                                  "READY FOR FILES ");
          }
          SSD1306_UpdateScreen();
  ...
  /* No host was present during the startup window. FatFS takes ownership and
     USB reports no medium until a later safe logging-to-USB transition. */
  USB_Storage_SetReady(0);
  if (usb_clock_enabled)
  {
      if (SD_SetTransferClock(16) != HAL_OK)
      {
          SSD1306_Clear();
  ...
  while (1)
  {
      if (hUsbDeviceFS.dev_state == USBD_STATE_CONFIGURED)
      {
          ...
          /* Reconfigure through HAL while neither FatFS nor USB is using SD. */
          ...
          /* USB owns the SD card until reset. This avoids treating selective
             suspend as a physical disconnect and remounting behind the host. */
          while (1)
          {
              uint32_t usb_blocks;
              uint32_t usb_errors;
